...
- DCA uses three reviewed checkpoint-area anchors. SFO uses five reviewed terminal-building anchors and groups six published checkpoints beneath them. EWR groups five readings under Terminals A, B, and C; LGA maps its Terminal B and C readings. All four use terminal + checkpoint mode without inheriting LAS-specific gate rules.
- The other 12 tracked airports use checkpoint-first mode: one airport-overview anchor, official feed checkpoint labels, Standard/PreCheck comparison, and no invented terminal, gate, or indoor checkpoint geometry.
...
flowchart LR
  ...
  Arrival --> TerminalCheckpoint["DCA / SFO / EWR / LGA terminal + checkpoint routing"]
  Arrival --> Generic["12 checkpoint-first airport views"]
