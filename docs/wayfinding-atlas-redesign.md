...
- DCA uses three reviewed checkpoint-area anchors. DEN maps East and West Security on the Jeppesen level. IAD maps East, West, and TSA PreCheck in the main terminal. IAH maps five terminals with published north/south splits where the airport page exposes them. BWI maps four checkpoint areas from the homepage security widget. DTW maps McNamara and Evans as terminal-level arrival targets from the public wait proxy. SFO uses five reviewed terminal-building anchors and groups six published checkpoints beneath them. EWR groups five readings under Terminals A, B, and C; LGA maps its Terminal B and C readings. BOS groups seven named checkpoints under Terminals A, B, C, and E; ORD groups its terminal-level feed under Terminals 1, 2, 3, and 5. DFW maps all five terminals and its 15 named checkpoint areas; PHL maps six terminal/checkpoint areas, including its shared D/E entry. MIA groups its ten numbered checkpoints plus DFIS under North, Central, and South terminal complexes, with live values only where the feed supplies them. LAX maps eight terminal areas and treats non-TBIT checkpoints as published routing context because the official live page currently reports only TBIT. ATL maps five checkpoint areas under the Domestic and International terminals; CLT maps three checkpoint areas under the single terminal complex; JAX maps one central terminal checkpoint; SEA maps six checkpoint areas under the published dashboard; JFK maps five terminal checkpoints; MCO maps three checkpoint areas around the north/south/east departures. All twenty-two use terminal + checkpoint mode without inheriting LAS-specific gate rules.
- The generic airport-overview builder remains available for future research airports, but no tracked live airport uses checkpoint-first mode now.
...
flowchart LR
  ...
  Arrival --> TerminalCheckpoint["DCA / DEN / IAD / IAH / BWI / DTW / SFO / EWR / LGA / BOS / ORD / DFW / PHL / MIA / LAX / ATL / CLT / JAX / SEA / JFK / MCO terminal + checkpoint routing"]
  Arrival --> Generic["generic fallback for future research airports"]
