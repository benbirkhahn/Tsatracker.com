...
- DCA uses three reviewed checkpoint-area anchors. SFO uses five reviewed terminal-building anchors and groups six published checkpoints beneath them. EWR groups five readings under Terminals A, B, and C; LGA maps its Terminal B and C readings. BOS groups seven named checkpoints under Terminals A, B, C, and E; ORD groups its terminal-level feed under Terminals 1, 2, 3, and 5. All six use terminal + checkpoint mode without inheriting LAS-specific gate rules.
- The other 10 tracked airports use checkpoint-first mode: one airport-overview anchor, official feed checkpoint labels, Standard/PreCheck comparison, and no invented terminal, gate, or indoor checkpoint geometry.
...
flowchart LR
  ...
  Arrival --> TerminalCheckpoint["DCA / SFO / EWR / LGA / BOS / ORD terminal + checkpoint routing"]
  Arrival --> Generic["10 checkpoint-first airport views"]
